...
bottom_decile =[i for j,i in enumerate(fsorted_data) if j in range(0, len(fsorted_data)/10)]
...
with open(sys.argv[2], "a") as myfile:
	...
	for company in top_decile:
		myfile.write(company[idx['nCompany Name']] +"  "+str(company[idx['cPB on NSE']])+"  "+str(company[-1]) + "\n")
	myfile.write("\nshort on\n" )
